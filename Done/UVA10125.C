 ...
 *
 * This program uses the meet-in-the-middle approach.
 * After reading in the set of numbers, it finds all A+B and stores them in
 *    sorted order, then stores all D-C in sorted order.
 *    Each list is made of structs that store three numbers: A/C, B/D, and
 *    the sum/difference of the two
 * Loop through all D-C, and do a binary search on the list of A+B numbers
 * If a match is found, make sure A, B, C, and D are unique, then print out D
 */
...
int main()
{
	...
	while (N != 0){
		...
		for (; rDiffIt != cdDiffs.rend(); ++rDiffIt){
			...
			if (match > -1){
				if (abSums[match].isValid(*rDiffIt))
